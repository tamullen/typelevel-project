...
object Doobie extends IOApp.Simple {
  ...
  // Functional programming library based on CE to interact with databases
  case class Student(id: Int, name: String)

  // need a transactor to interact with database
  ...
  def findStudentsByInitialLetter(letter: String): IO[List[Student]] = {
    ...
    val wherePart = fr"where left(LOWER(name), 1) = LOWER($letter)"
  ...
  // organize code
  // define type class that holds all the methods for one table
  // create a companion object with a factory method that will instantiate the type class
  ...
  val smallProgram = postgresResource.use { xa =>
    ...
    for {
      id <- studentsRepo.create("Travis")
      travis <- studentsRepo.findById(id)
      _ <- IO.println(s"the first student of Rock the JVM is $travis")
    } yield ()
  }
