...
import cats.effect.kernel.{Deferred, Fiber, Spawn}
import cats.effect.{Concurrent, GenSpawn, IO, IOApp, MonadCancel, Ref, Resource, Sync, Temporal}
...
//  }
  ...
  trait MyGenSpawn[F[_], E] extends MonadCancel[F, E] {
    def start[A](fa: IO[A]): F[Fiber[F, E, A]] // creates a fiber
    // never, cede, racePair
  }
  ...
  trait MySync[F[_]] extends MonadCancel[F, Throwable] with Defer[F] {
    ...
    def blocking[A](expression: => A): F[A ]// runs effect on dedicated blocking thread pool
  }
